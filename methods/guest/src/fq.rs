use ark_ff::{
    FftParameters, FpParameters,
};

use crate::biginteger::{BigInteger256_32 as BigInteger, BigInteger256_32};
...
macro_rules! impl_Fp {
    ($Fp:ident, $FpParameters:ident, $BigInteger:ident, $BigIntegerType:ty, $limbs:expr, $field_size:expr) => {
        ...
        impl<P> $Fp<P> {
            ...
            /// of this method
            #[doc(hidden)]
            pub const fn const_from_str(limbs: &[u32], is_positive: bool, r2: $BigIntegerType, modulus: $BigIntegerType, inv: u32) -> Self {
            ...
            #[inline]
            pub(crate) const fn const_from_repr(repr: $BigIntegerType, r2: $BigIntegerType, modulus: $BigIntegerType, inv: u32) -> Self {
            ...
            #[ark_ff_asm::unroll_for_loops]
            const fn mul_without_reduce(mut self, other: &Self, modulus: $BigIntegerType, inv: u32) -> Self {
            ...
            #[ark_ff_asm::unroll_for_loops]
            const fn const_mul(mut self, other: &Self, modulus: $BigIntegerType, inv: u32) -> Self {
        ...
        impl<P: $FpParameters> Field for $Fp<P> {
            ...
            fn from_random_bytes_with_flags<F: Flags>(bytes: &[u8]) -> Option<(Self, F)> {
                if F::BIT_SIZE > 8 {
                    ...
                    let last_limb_mask = (u32::MAX >> P::REPR_SHAVE_BITS).to_le_bytes();
                ...
                for c in s.chars() {
                    match c.to_digit(10) {
                        Some(c) => {
                            ...
                            res.mul_assign(&ten);
                            let digit = Self::from(u32::from(c));
...
impl FftParameters for FqParameters {
    ...
    // ]);
    // convert above 4 limbs of 64 bits each to 8 limbs of 32 bits
