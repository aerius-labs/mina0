...
use risc0_zkvm::{serde::to_vec, compute_image_id, default_prover, ExecutorEnv, Receipt};
...
fn main() {
    ...
    let result = if std::env::var("RISC0_PROVER").unwrap_or(String::new()) == "bonsai" {
        run_bonsai(ctx_with_proof)
    } else {
        ...
        let receipt = prover.prove(env, KIMCHI0_ELF).unwrap();
    ...
    println!("time to prove: {:?}", end - start);
    result.unwrap();
...
fn run_bonsai(input_data: ContextWithProof) -> Result<()> {
    ...
    let img_id = {
        let image_id = hex::encode(compute_image_id(KIMCHI0_ELF)?);
        client.upload_img(&image_id, KIMCHI0_ELF.to_vec())?;
        image_id
    };
    ...
    let session = client.create_session(img_id, input_id, vec![])?;
    loop {
        ...
        if res.status == "SUCCEEDED" {
            ...
            if res.error_msg.is_some() {
                ...
                return Err(anyhow::anyhow!("Workflow exited: {}", res.error_msg.unwrap()));

            } else {
                // panic!("Workflow exited: {}", res.status);
                // return error instead of panicking
                return Err(anyhow::anyhow!("Workflow exited: {}", res.status));
            }
        }
